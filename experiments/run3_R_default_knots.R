# Run 3: R smooth.spline with default reduced knots (t != x), GCV vs exact LOOCV.
...
for (j in seq_along(lams)) {
  ...
  fit <- smooth.spline(x, y, all.knots = FALSE, lambda = l)
  ...
  for (k in 1:n) {
    ek <- numeric(n)
    ek[k] <- 1
    A[, k] <- predict(smooth.spline(x, ek, all.knots = FALSE, lambda = l), x)$y
  }
...
write.csv(data.frame(lam = lams, trA = trA_all, V_gcv = v_gcv, V_loo = v_loo),
          "run3_scores.csv", row.names = FALSE)

png("run3_curves.png", width = 960, height = 720, res = 150)
plot(lams, v_gcv, log = "xy", type = "l", col = "blue",
     xlab = "lambda (R scale)", ylab = "score",
     main = "Run 3: R default knots (t != x), GCV vs exact LOOCV")
